...
//        echo $m;die;
        phpinfo();die;
        $sql = 'select * from tb_area_config limit 2';
        $res = DB::select($sql);
        $newRes = [];
        foreach ($res as $row)
        {
    ...
    {
        $path = rtrim(dirname(base_path(), 2), '/').'/log/_nsm_xml_log';
        ...
        $itr = new \RecursiveIteratorIterator(new RecursiveFilterFileIterator($dir,
            ''),
    ...
    {
        $res = DB::select('select distinct province_id,province from day_area_nsm');
        dump($res);
        die;
    }
  ...
  "city": [
      ...
      }, 
      {
        ...
        foreach ($arr['city'] as $item) {
            foreach ($item['list'] as $row) {
                $insertData[] = [
                    'name'  => $row['name'],
                    'pinyin'=> substr($row['pinyin'], 0, 1)
                ];
            }
        }
        $res = DB::table('tb_city_code')->insert($insertData);
        echo 'insert success!';
    }
    ...
    public function index6()
    {
        ...
        $nameArr = DB::table('tb_city_code')->pluck('name')
        ->toArray();
        $nameArr = array_map(function($value) {
            return "'". $value. "'";
        }, $nameArr);
        $re = DB::select("SELECT distinct city_id,city from day_area_nsm where city IN(".implode(',', $nameArr).")")
        ;
        $map = [];
        foreach ($re as $row) {
            $map[$row->city] = $row->city_id;
        }

        $re = DB::select('select id, name from tb_city_code');
        foreach ($re as $row) {
            $name = $row->name;
            if (isset($map[$name])) {
                $cityId = $map[$name];
                DB::table('tb_city_code')->where('id', $row->id)
